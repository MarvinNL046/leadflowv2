...
export function SuiteAccess({
	...
	return (
		<section id="bedrijfsaccounts" className="space-y-4">
			...
			{status?.canPair && (
				<div className="flex flex-wrap gap-2">
					{(["frostwork", "cashflow"] as const)
						.filter(
							(product) => !status.bindings.some((b) => b.product === product),
						)
						.map((product) => (
							<Button
								key={product}
...
function AccessRow({
	...
	const now = useStatusTime();
	const accessStatus = suiteStatus(row, now);
	...
	return (
		<Card>
			<CardContent className="space-y-3 p-4">
				...
				<p className="text-sm">
					Bedrijfsaccount gekoppeld. Aanvraaggoedkeuring geeft nog geen
					producttoegang. Ken die hieronder afzonderlijk toe met de afgesproken
					einddatum.
				</p>
				<details>

...
export default defineSchema({
  suitePairingCodes:defineTable({orgId:v.id('orgs'),product:v.union(v.literal('frostwork'),v.literal('cashflow')),userId:v.id('users'),subject:v.string(),issuer:v.string(),hash:v.string(),expiresAt:v.number()}).index('by_orgId_and_product',['orgId','product']).index('by_hash',['hash']),
  suiteBindings:defineTable({orgId:v.id('orgs'),product:v.union(v.literal('frostwork'),v.literal('cashflow')),targetOrgId:v.string(),pairedBy:v.id('users'),pairedAt:v.number(),revision:v.number(),enabled:v.boolean(),validUntil:v.number()}).index('by_orgId_and_product',['orgId','product']).index('by_product_and_targetOrgId',['product','targetOrgId']),
  suiteAccessAudit:defineTable({bindingId:v.id('suiteBindings'),actor:v.id('users'),changedAt:v.number(),enabled:v.boolean(),validUntil:v.number(),note:v.string()}).index('by_bindingId',['bindingId']),
  appRequestReviews:defineTable({requestId:v.id('appRequests'),actor:v.id('users'),reviewedAt:v.number(),status:v.union(v.literal('approved'),v.literal('rejected')),note:v.string(),revision:v.number()}).index('by_requestId',['requestId']),
  appRequests:defineTable({orgId:v.id('orgs'),product:v.union(v.literal('frostwork'),v.literal('cashflow')),requestedBy:v.id('users'),requestedAt:v.number(),status:v.optional(v.union(v.literal('pending'),v.literal('approved'),v.literal('rejected'))),reviewNote:v.optional(v.string()),reviewedAt:v.optional(v.number()),reviewedBy:v.optional(v.id('users')),revision:v.optional(v.number())}).index('by_orgId_and_product',['orgId','product']),
  ...
  messages: defineTable({
    ...
    smsConnectionId:v.optional(v.id('companySmsConnections')),
    to: v.string(),
    ...
    .index('by_smsConnectionId_and_externalMessageId',['smsConnectionId','externalMessageId']).index('by_whatsappConnectionId_and_externalMessageId',['whatsappConnectionId','externalMessageId']).index("by_external_id", ["externalMessageId"])
  ...
  marketplaceServiceReviews: defineTable({
    leadId: v.id('marketplaceLeads'),
    before: v.union(marketplaceServiceType,v.null()), after: v.union(marketplaceServiceType,v.null()),
    reviewedBy: v.id('users'), reviewedAt: v.number(), note: v.string(), revision: v.number(),
  }).index('by_lead',['leadId']),
